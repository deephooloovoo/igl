...
/* IGL: (see panel.h) writing to read-only memory segment is unwanted nowadays */
char pnl_scriptinfile[1024];
char pnl_scriptoutfile[1024];


#ifdef IRIS_4D
Actuator *pnl_pickact(Panel *, Coord, Coord);
static Boolean _hitact(Actuator *, Panel *, Coord, Coord);	/* note: can't declare floats */
static void _delact(Actuator *a);
static void _dumpact(Actuator *, int);
static void _loadact(Actuator *, int);
static void _delpnl(Panel *p);
static void _fixpnl(Panel *p);
static void _drawpnl(Panel *p);
#else  IRIS_4D
...
void
pnl_labeldimensions(Actuator *a)
{
  if (!a->p) return;
...
void
labeldimensions(Actuator *a)
{
  pnl_labeldimensions(a);
}

void
pnl_setdirty(Actuator *a)
{
  Actuator *b;
...
void
pnl_addtogroup(Actuator *a, Panel *p)
{
...
void
addtogroup(Actuator *a, Panel *p)
{
  pnl_addtogroup(a, p);
}

void
pnl_endgroup(Panel *p)
{
  p->lastgroup=NULL;
}
...
void
pnl_newvalact(Actuator *a, Panel *p, Coord x,Coord y)
{
  pnl_tmpwin=0;
...
void
newvalact(Actuator *a, Panel *p, Coord x,Coord  y)
{
  pnl_newvalact(a, p, x, y);
}

void
pnl_fixpanel(Panel *p)
{
  if (p&&p->fixfunc) {
    (*p->fixfunc)(p);
  }
  p->dirtycnt = 2;
#ifdef DEBUG
  fprintf (stdout,"pnl_fixpanel: minx:%f, y:%f, maxx:%f, y:%f\n",
...
pnl_labeloffsets(Actuator *a)
{
  ...
  case PNL_LABEL_LOWER_LEFT:
    ...
    break;
  default:
    (void) fprintf (stdout,"unknown label type %d\n", a->labeltype);
    exit(1);
  }
}

void
labeloffsets(Actuator *a)
{
  pnl_labeloffsets(a);
}

void
pnl_panellimitsact(Actuator *a, float sf, Coord x,Coord y)
...
void
panellimitsact(Actuator *a,float sf,Coord x,Coord y)
{
...
void
reshapepanel(Panel* p)
{
  pnl_reshapepanel(p);
}

void
pnl_initpanel(Panel *p)
{
...
void
pnl_mousetoworld(Panel* p, Screencoord mx,Screencoord  my, Coord *x,Coord *y)
{
  if (p&&p->gid) {
    winset(p->gid);
...
void
mousetoworld(Panel* p, Screencoord mx,Screencoord  my, Coord *x,Coord *y)
{
  pnl_mousetoworld(p, mx, my, x, y);
}

void
pnl_worldtomouse(Panel *p,Coord x,Coord y, Screencoord *mx, Screencoord *my)
{
  if (p) {
...
void
worldtomouse(Panel *p,Coord x,Coord y, Screencoord *mx, Screencoord *my)
{
...
short data;
	...
	if (!p) {
#ifdef DEBUG
(void) fprintf (stdout,"pnl_userredraw consuming a REDRAW for mex window %d\n",data);
#endif DEBUG
	    return data;
	} else {
#ifdef DEBUG
(void) fprintf (stdout,"pnl_userredraw requeueing a REDRAW for panel %s\n",p->label);
#endif DEBUG
	    qenter(dev,data);
	    return 0;
	}
...
Boolean
pnl_beginwritescript(char* name)
{
...
void
pnl_getnewactive(Actuator **ca,Panel **cp,Screencoord mx,Screencoord my)
{
  Panel *p=NULL;
  Actuator *a=NULL;
  ...
  if (p&&!p->selectable) {
#ifdef DEBUG
    fprintf (stdout,"a hit on unselectable panel %s\n", p->label?p->label:"<no label>");
#endif DEBUG
...
#ifdef DEBUG
  if (p) (void) fprintf (stdout,"a hit on panel %s\n", p->label);
  else   (void) printf
...
#ifdef DEBUG
      (void) fprintf (stdout,"picked the mouse actuator\n");
#endif DEBUG
      p=NULL;
      a=pnl_mouse_act;
      a->active=TRUE;
...
void
getnewactive(Actuator **ca, Panel **cp,Screencoord mx,Screencoord my)
{
  pnl_getnewactive(ca,cp,mx,my);
}

void
pnl_dofuncs(Actuator *a,Panel *p)
{
...
void
pnl_setinactive(Actuator *a,Panel *p)
{
  if (p) p->active=FALSE;
  if (a) {

#ifdef DEBUG
    (void) fprintf (stdout,"unpicked %s\n", a->label);
#endif DEBUG
...
void
setinactive(Actuator *a,Panel *p)
{
  pnl_setinactive(a,p);
}

void
pnl_doautos(Panel *p,Coord x,Coord y)
{
  Alist *e;
...
void
doautos(Panel *p,Coord x,Coord y)
{
  pnl_doautos(p,x,y);
}
Boolean
 ...
 redo:
  if (pnl_block&&!(pnl_ca&&!pnl_activetypein)||qtest()) {
...
#ifdef DEBUG
      (void) fprintf (stdout,"consumed a PNL_TOKEN_REDRAW, pnl_dopanel returning\n");
#endif DEBUG
#ifdef IRIS_4D
    ...
    case REDRAW:
 	...
 	if (p->gid==data) {
#ifdef DEBUG
	  fprintf (stdout,"redraw for panel %s\n", p->label?p->label:"<no label>");
#endif DEBUG
	  pnl_reshapepanel(p);
	  pnl_calcppu(p);
 	  break;
...
#ifdef DEBUG
 	(void) fprintf (stdout,"enqueing a PNL_TOKEN_REDRAW\n");
#endif DEBUG
...
pnl_forkpanel()
{
/* IGL: sproc() not supported, none of the working demos use pnl_forkpanel()
...
Actuator *pnl_pickact(Panel *p, Coord x, Coord y)
{
  Actuator *a;
...
Actuator *pickact(Panel *p,Coord x,Coord y)
{
  return pnl_pickact(p,x,y);
}

static Boolean
_hitact(Actuator *a,Panel *p,Coord x,Coord y)
{
  return PNL_HITACT(a,x,y);
}

static void
_dumpact(Actuator *a, int fd)
{
...
Panel *p;
{
  ...
  if (!p->visible) {
    if (p->gid) {
#ifdef DEBUG
      fprintf (stdout,"fixpanel: closing window\n");
#endif DEBUG
      winclose(p->gid);
      pnl_tmpwin=0;	/* this variable communicates with newvalact() */
      p->gid=0;
...
#ifdef DEBUG
    fprintf (stdout,"fixpanel: new window\n");    
#endif DEBUG
    prefposition(p->x, p->x+p->w, p->y, p->y+p->h);
    p->gid=winopen("control panel");
    wintitle(p->label);
...
#ifdef DEBUG
      fprintf (stdout,"fixpanel: reshape\n");      
#endif DEBUG
      winposition(p->x, p->x+p->w, p->y, p->y+p->h);
      pnl_reshapepanel(p);
    } else if (p->x!=x||p->y!=y) { 	/* just moved */
#ifdef DEBUG
      fprintf (stdout,"fixpanel: moved\n");      
#endif DEBUG
      winposition(p->x, p->x+p->w, p->y, p->y+p->h);
    } else {	    			/* maybe just made a new actuator */
#ifdef DEBUG
      fprintf (stdout,"fixpanel: other\n");      
#endif DEBUG
      ominx=p->minx;
      ominy=p->miny;
      omaxx=p->maxx;
...
void
pnl_drawfade(Actuator *a, Panel *p)
{
...
void
pnl_drawact(Actuator *a, Panel* p)
{
  ...
  _drawpnl(Panel *p)
{
  ...
  if (p->dirtycnt) {
    winset(p->gid);
#ifdef DEBUG
    (void) fprintf (stdout,"drawing panel %s\n",  p->label);
#endif DEBUG
#ifndef PNL_ZBUFFER
    color(pnl_background_color);
    clear();
...
void
pnl_drawlabel(Actuator *a,Panel *p)
{
  pushmatrix();
  PNL_ZTRANSLATE;
  PNL_ZTRANSLATE;
...
void
pnl_drawbevel(Actuator *a, Panel *p)
{
  float p1x = a->x;
  float p1y = a->y;
  float p2x = a->x;
