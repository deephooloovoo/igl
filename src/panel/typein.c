...
int
  _getnextchar(char *string, int maxstringlen)
{
  int getcharaction();
  register int i;
  int c;
...
void _newvaltypein(Actuator *a,Panel *p, Coord x, Coord y)
{
  int retval;
...
void
  _drawtypein(Actuator *a, Panel *p)
{
  Typein *ad=(Typein *)a->data;
