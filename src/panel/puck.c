...
void _newvalpuck(Actuator *a, Panel *p,Coord x,Coord y)
{
...
void
_drawpuckshape(Actuator *a, Panel *p, Coord x, Coord  y, int  style)
{
static Coord d=PNL_PUCK_SIZE/2.0;
...
void
  _newvalfloatingpuck(Actuator *a, Panel *p, Coord x, Coord y)
{
  static Coord d=PNL_PUCK_SIZE/2.0;
...
static Coord ox=0.0,oy=0.0,ox2=0.0,oy2=0.0;
	...
	if (mode==PUCK_DRAW
	    ...
	    && drawn) return;
#ifdef IRIS_4D
/* IGL: drawmode() not supported, because we don't have layer support yet
...
void
_drawfloatingpuck(Actuator *a, Panel *p)
{
static Coord d=PNL_PUCK_SIZE/2.0;
Coord x=0.0, y=0.0;
...
void
_drawrubberband(Actuator *a, Panel *p, Coord x,Coord  y, int style)
{
float r, alpha;
Coord dx, dy;
    ...
    pushmatrix();
    r=hypot((long double)dx,(long double)dy);
...
void
_drawrubberpuckshape(Actuator *a, Panel *p, Coord x, Coord y, int style)
{
static Coord d=PNL_PUCK_SIZE/2.0;
...
void
_drawrubberpuckshapeandband(Actuator *a, Panel *p, Coord x, Coord y, int style)
{
...
void
_drawrubberpuck(Actuator *a, Panel *p)
{
static Coord d=PNL_PUCK_SIZE/2.0;
static Coord x, y;
